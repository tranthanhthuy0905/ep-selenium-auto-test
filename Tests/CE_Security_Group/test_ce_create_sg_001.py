'''
Screnario 1: Create Security Group With Simple Flow
...
from Pages.CE.security_group_page import SGHomePage
from Tests.CE.ce_base_test import CEBaseTest


class AuditCreateSecurityGroup(CEBaseTest):

    def create_security_group(self):
        self.sg_home_page = SGHomePage(self.driver)
        self.sg_home_page.click_create_button()

        self.assertEqual(self.driver.current_url, self.sg_home_page.base_url)

        self.sg_home_page.fill_sg_information()
        self.sg_home_page.click_create_submit_button()
